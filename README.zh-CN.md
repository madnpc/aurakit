...
- 灯效 type 映射仍在验证中，目前已从导出文件确认 `5 = Comet`、`7 = Tide`，以及 `11` 到 `13` 的信号同步类型。
...
| 1 | 呼吸 | Breathing | 根据 UI 顺序推断；已验证双色模式 |
| 2 | 彩色循环 | Color Cycle | 根据 UI 顺序推断 |
| 3 | 彩虹 | Rainbow | 根据 UI 顺序推断 |
| 4 | 闪烁 | Flash | 根据 UI 顺序推断；已验证双色模式 |
| 5 | 彗星 | Comet | 已通过导出时间轴验证 |
| 6 | 繁星 | Starry Night | 根据 UI 顺序推断 |
| 7 | 潮汐 | Tide | 已通过导出时间轴验证 |
| 11 | 音乐 | Music signal sync | 已通过导出 fixture 验证 |
| 12 | 智能 | Smart signal sync | 已通过导出 fixture 验证 |
| 13 | 同步变色 | Synchronized color change | 已通过导出 fixture 验证 |
